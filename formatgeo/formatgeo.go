...
func (*T) Detect(ls []string) bool {
	return len(ls) > 0 && idRegExp.FindString(ls[0]) != ""
}

func (*T) Read(ls []string) (waypoint.Collection, error) {
	var wc waypoint.Collection
	for _, l := range ls[1:] {
		ss := regExp.FindStringSubmatch(l)
		if ss == nil {
			continue
		}
...
func (*T) Write(w io.Writer, wc waypoint.Collection) error {
	...
	for _, wp := range wc {
		...
		if _, err := fmt.Fprintf(w, "%-8s  %s %02d %02d %05.2f    %s %03d %02d %05.2f  %4d  %s\r\n", wp.Id, latHemi, latDeg, latMin, latSec, lngHemi, lngDeg, lngMin, lngSec, wp.Altitude, wp.Description); err != nil {
			return err
		}
	}
	return nil
